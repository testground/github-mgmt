# GitHub Management via Terraform: testground

This repository is responsible for managing GitHub configuration of `testground` organisation as code with Terraform. It was created from [github-mgmt-template](https://github.com/protocol/github-mgmt-template) and it will receive updates from that repository.

## Managed resources

- `membership`: `testground` members
  - `role`
- `repository`: repositories owned by `testground`
- `repository_collaborator`: direct user collaborators of a repository per repository
  - `permission`
- `team`: teams owned by `testground`
